...
export class AppController {
  constructor(dependencies) {
    ...
    this.state.subscribe('currentView', (newView) => {
      ...
      const currentActiveId =
        Object.keys(viewsEl).find((key) => viewsEl[key].classList.contains('active')) || 'checkin';
  ...
  async handleLike() {
    ...
    if (trackToUpdate.interactions) {
      trackToUpdate.interactions.likes = Math.max(
        0,
        trackToUpdate.interactions.likes + (isLiked ? -1 : 1)
      );
    } else {
    ...
    this.views.rating.render(
      trackToUpdate.interactions,
      !isLiked,
      this.state.get('ratedTracks')[trackId] || 0
    );
